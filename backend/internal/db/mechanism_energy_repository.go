...
type MechanismEnergy struct {
	...
	ContractPrice  decimal.Decimal `json:"contract_price"` // P4: numeric(18,4)
	SettleAmount   decimal.Decimal `json:"settle_amount"`  // P4: numeric(18,4)
...
func (r *MechanismEnergyRepository) GenerateDemo(ctx context.Context) (int, error) {
	...
	for i := 0; i < 12; i++ {
		...
		for _, v := range volts {
			...
			dev := actual - planned
			// P4: 金额 decimal（settle = actual*price）；电量保持 float。
			price := decimal.NewFromFloat(basePrice[v]).Round(4)
			settle := decimal.NewFromFloat(actual).Mul(price).Round(4)
