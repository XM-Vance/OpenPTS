...
func registerEnhance(g *gin.RouterGroup, d *Deps) {
	...
	g.DELETE("/custom-fields/:id", reqSYWrite, customFieldH.Delete)
	...
	g.DELETE("/custom-fields/values", reqCMWrite, customFieldH.DeleteValue)

	// 标签管理
	g.GET("/tags", reqSYRead, tagH.List)
	g.POST("/tags", reqSYWrite, tagH.Create)
	g.PUT("/tags/:id", reqSYWrite, tagH.Update)
	g.DELETE("/tags/:id", reqSYWrite, tagH.Delete)
